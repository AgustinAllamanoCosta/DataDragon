...
class Blockchain:
  ...
  def toDictionary(chain):
    ...
    for block in chain:
      if block.data:
        chain_to_dictionary.append({ "timestamp": block.timestamp, "lastHash": block.lastHash, "hash": block.hash, "data": base64.b64decode(block.data[1:]).decode("utf-8"),  "dataIndex": block.dataIndex,  "filename": block.filename,  "validationHash": block.validationHash, "validator": block.validator, "signature": block.signature })
      else:
